...
TEST_F(AnalyzerWaveformTest, canary) {
    ...
    ConstWaveformPointer pWaveform = m_pTrack->getWaveform();
    ASSERT_NE(pWaveform, nullptr);
    EXPECT_GT(pWaveform->getDataSize(), 0);
    EXPECT_EQ(pWaveform->getCompletion(), pWaveform->getDataSize());
...
TEST(WaveformFactoryTest, separatesLegacyAndPerceptualCacheProfiles) {
    EXPECT_EQ(WaveformFactory::waveformVersionToVersionClass(
                      ...
                      WaveformAnalysisProfile::Legacy),
    ...
    EXPECT_EQ(WaveformFactory::waveformVersionToVersionClass(
                      ...
                      WaveformAnalysisProfile::Perceptual3Band),
            WaveformFactory::VC_USE);
    EXPECT_EQ(WaveformFactory::waveformSummaryVersionToVersionClass(
    ...
    EXPECT_EQ(WaveformFactory::waveformVersionToVersionClass(
            ...
            WaveformFactory::VC_REMOVE);
    EXPECT_EQ(WaveformFactory::waveformVersionToVersionClass(
            QStringLiteral("Waveform-RMS-obsolete-1"),
                      WaveformAnalysisProfile::Legacy),
            WaveformFactory::VC_REMOVE);
}
