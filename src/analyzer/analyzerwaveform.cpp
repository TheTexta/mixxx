...
AnalyzerWaveform::AnalyzerWaveform(
        ...
        : m_pConfig(pConfig),
          m_analysisProfile(WaveformAnalysisProfile::Legacy),
...
bool AnalyzerWaveform::initialize(const AnalyzerTrack& track,
    ...
    // The waveform memory is initialized now, so other threads may access it.
    ...
    if (m_analysisProfile == WaveformAnalysisProfile::Perceptual3Band) {
...
void AnalyzerWaveform::createFilters(mixxx::audio::SampleRate sampleRate) {
    ...
    // Settle filters for silence in preroll to avoid ramping (Issue #7776).
...
bool AnalyzerWaveform::processSamples(const CSAMPLE* pIn, SINT count) {
    ...
    const bool isPerceptual = m_analysisProfile == WaveformAnalysisProfile::Perceptual3Band;
...
void AnalyzerWaveform::storeResults(TrackPointer pTrack) {
    ...
    // Set waveforms on track after they have been written to disk so overview
    // cache reads a consistent waveform summary.
